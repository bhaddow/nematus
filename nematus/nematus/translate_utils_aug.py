...
def translate_file(input_file, output_file, session, sampler, config,
                   ...
                   mask='0', extended_translations=None):
    ...
    def translate_maxibatch_slt(maxibatch, num_to_target, target_to_num, num_prev_translated, last_translation, mask='0', trans=[], last_line=''):
        """Translates an individual maxibatch.
    ...
    if strategy == 'biased_beam_search':
        ...
        while True:
            ...
            if len(maxibatch) == (maxibatch_size * minibatch_size):
                ...
                if not prefix_mask[sent_id]:
                    # print('mask works!')
                    last_translation, last_textline = translate_maxibatch_slt(maxibatch, num_to_target, target_to_num, num_translated, last_translation, mask=mask, trans=[extension[sent_id] for extension in extensions], last_line=last_textline)
                else:
                    last_translation, _ = translate_maxibatch_slt(maxibatch, num_to_target, target_to_num, num_translated, last_translation, mask='0')
        ...
        while True:
            ...
            if len(maxibatch) == (maxibatch_size * minibatch_size):
                if not prefix_mask[sent_id]:
                    translate_maxibatch(maxibatch, num_to_target, target_to_num, num_translated, mask=mask)
                else:
                    translate_maxibatch(maxibatch, num_to_target, target_to_num, num_translated, mask='0')
                # translate_maxibatch(maxibatch, num_to_target, num_translated)
                num_translated += len(maxibatch)
                maxibatch = []
            sent_id += 1
