...
def _beam_search(model_adapters, beam_size, batch_size_x, max_translation_len,
                 normalization_alpha, vocab_size, eos_id, last_translation, last_translation_len, beta):
    ...
    loop_vars = [current_time_step,
                 ...
                 finished_eos_flags,
                 alive_memories]
    # print(alive_scores.get_shape(), finished_scores.get_shape())
    shape_invariants=[
...
def _generate_while_loop_body_func(model_adapters, decoding_functions,
                                   ...
                                   last_translation, last_translation_len, beta):
    ...
    def biased_extend_hypotheses(current_time_step, alive_sequences, alive_scores,
                          alive_memories, last_translation, last_translation_len):
        """Generates top-k extensions of the alive beam candidates."""

        # Get the vocab IDs for this timestep in the order of the model inputs.
        # alive_sequences = tf.Print(alive_sequences, [alive_sequences], message='alive_seq:\t')
        next_ids = alive_sequences[:, :, -1]      # [batch_size_x, beam_size]
        next_ids = tf.transpose(next_ids, [1, 0]) # [beam_size, batch_size_x]
        next_ids = tf.reshape(next_ids, [-1])     # [beam_size * batch_size_x]
        ...
        def interpolation(sum_log_probs, vocab_id):
            # beta = self.beta
            prob = tf.math.exp(sum_log_probs)
            return tf.math.log((1-beta)*prob + tf.one_hot(vocab_id, vocab_size, on_value=beta))
            # return sum_log_probs

        def biased_interpolation(sum_log_probs, current_time_step, alive_sequences, last_translation):
            last_trans = last_translation[:current_time_step]
            # print(sum_log_probs.shape)
            # print(last_translation.shape)
            output_list = tf.unstack(sum_log_probs, num = 1)
            beam_list = tf.unstack(output_list[0])
            for i in range(beam_size):
                seq = alive_sequences[0, i, :]
                beam_list[i] = tf.cond(tf.reduce_all(tf.equal(seq, last_trans)), \
                    lambda: interpolation(sum_log_probs[0, i, :], last_translation[current_time_step]), \
                    lambda: unbias(sum_log_probs[0, i, :]))
                # output_list[0].append(posterior)
            # print(output_list)
            output_list[0] = tf.stack(beam_list)
            sum_log_probs = tf.stack(output_list)
            sum_log_probs = tf.reshape(sum_log_probs, [batch_size_x, beam_size, vocab_size])
            # print(sum_log_probs.shape)
            # sum_log_probs = tf.Print(sum_log_probs, [sum_log_probs], message='Prob:\t')
            return sum_log_probs

        def unbias(sum_log_probs):
            return sum_log_probs
        
        sum_log_probs = tf.cond(tf.greater(last_translation_len, current_time_step), \
             lambda: biased_interpolation(sum_log_probs, current_time_step, alive_sequences, last_translation),
             lambda: unbias(sum_log_probs)
             )
    ...
    def decoding_step(current_time_step, alive_sequences, alive_scores,
        ...
        # alive_sequences = tf.Print(alive_sequences, [alive_sequences], summarize=10)
        # if last_translation.get_shape()[0] <= alive_sequences.get_shape()[2]:
        #     top_sequences, top_scores, top_eos_flags, top_memories = \
        #         extend_hypotheses(current_time_step,
        #                           alive_sequences,
        #                           alive_scores,
        #                           alive_memories)
        #     last_translation = tf.Print(last_translation, [last_translation], summarize=10)
        # else:
        #     top_sequences, top_scores, top_eos_flags, top_memories = \
        #         biased_extend_hypotheses(current_time_step,
        #                           alive_sequences,
        #                           alive_scores,
        #                           alive_memories,
        #                           last_translation)
            # alive_sequences = tf.Print(alive_sequences, [alive_sequences], summarize=10)
        # top_sequences, top_scores, top_eos_flags, top_memories = \
        #     tf.cond(last_translation.get_shape()[0] <= current_time_step,
        #             lambda: extend_hypotheses(current_time_step,
        #                       alive_sequences,
        #                       alive_scores,
        #                       alive_memories),
        #             lambda: biased_extend_hypotheses(current_time_step,
        #                       alive_sequences,
        #                       alive_scores,
        #                       alive_memories,
        #                       last_translation))
        ...
        # alive = tf.Print(alive_sequences, [alive_sequences], summarize=10)
        # lenth = alive_sequences.get_shape()
